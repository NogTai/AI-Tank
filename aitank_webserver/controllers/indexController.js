...
exports.index_get = async (req, res) => {
  ...
  article.forEach((element) => {
    element.modifiedTime = moment(element.modifiedTime).format(
      "DD/MM/YYYY HH:mm:ss"
    );
  });
  ...
  Active.findById(req.params.id)
    .then(async (active) => {
      ...
      res.render("client/signup", {
        user: null,
        article: article,
        msg: "Kích hoạt tài khoản thành công, đăng nhập để tiếp tục!",
      });
    })
    .catch((err) => {
      res.send("404");
          ...
          if (result) {
            if (!user.status) {
              return done(null, false, {
                msg: "Vui lòng kiểm tra email và kích hoạt tài khoản!",
              });
            }
