...
exports.list = async function (req, res) {
  try {
    let userSort = await User.find({ role: "user" }).sort({
      win: -1,
      lose: 1,
      draw: -1,
    });
    ...
    for (let i = 0; i < userSort.length; i++) {
      userSort[i].ratio =
        userSort[i].win == userSort[i].win + userSort[i].lose + userSort[i].draw
          ? 100
          : parseInt(
              (userSort[i].win /
                (userSort[i].win + userSort[i].lose + userSort[i].draw)) *
                100
            );
      userSort[i].botSelected = await Helper.findBotSelectedByUserID(
        userSort[i]._id.toString()
      );
    }
    ...
    res.render("aigame/player", {
      user: null,
      data: { error: "Hiện tại chưa có Player!" },
    });
  }
};
  ...
  if (body) {
    ...
    try {
      ...
      if (result != "") {
        ...
        res.render("aigame/player", {
          user: user,
          data: { error: "Không tìm thấy Player!" },
        });
      }
    } catch (error) {
      console.log(error);
