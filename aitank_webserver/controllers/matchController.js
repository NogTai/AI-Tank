  ...
  if (params) {
    try {
      ...
      if (match.status == "pending") {
        res.render("aigame/matches", {
          user: null,
          data: { error: "Trận đấu chưa bắt đầu!" },
        });
      } else {
        let data;
        if (match.status == "started") {
          data = {
            g_replayFileName: "",
            g_serverPort: configuration.LIVE_STREAM_PORT,
...
exports.list_get = async function (req, res) {
  try {
    let matchs = await Match.find()
      .populate({ path: "botOne", populate: [{ path: "creator" }] })
      .populate({ path: "botTwo", populate: [{ path: "creator" }] })
      .sort({ createdTime: -1 });
    for (var i = 0; i < matchs.length; i++) {
      if (matchs[i].challenger == 1) {
        matchs[i].challengerName =
          matchs[i].botOne.creator.lastName +
          " " +
          matchs[i].botOne.creator.firstName +
          " (P1)";
        matchs[i].opponent =
          matchs[i].botTwo.creator.lastName +
          " " +
          matchs[i].botTwo.creator.firstName +
          " (P2)";
      } else {
        matchs[i].challengerName =
          matchs[i].botTwo.creator.lastName +
      ...
      if (matchs[i].result.code == 1) {
        ...
        matchs[i].winner =
          ...
          " " +
          matchs[i].botOne.creator.firstName +
          " (Do team 2 chọn nhiều tank hơn cho phép)";
      } else if (matchs[i].result.code == 6) {
        matchs[i].winner =
          matchs[i].botTwo.creator.lastName +
          " " +
          matchs[i].botTwo.creator.firstName +
          " (Do team 1 chọn nhiều tank hơn cho phép)";
      } else if (matchs[i].result.code == 7) {
        matchs[i].winner =
          matchs[i].botTwo.creator.lastName +
          " " +
          matchs[i].botTwo.creator.firstName +
          " (Do team 1 đặt tank ở vị trí bị cấm)";
      } else if (matchs[i].result.code == 8) {
        matchs[i].winner =
          matchs[i].botOne.creator.lastName +
          " " +
          matchs[i].botOne.creator.firstName +
          " (Do team 2 đặt tank ở vị trí bị cấm)";
    ...
    res.render("aigame/matches", {
      user: null,
      data: { error: "Chưa có trận đấu nào diễn ra!" },
    });
  }
};
    ...
    if (bot1 && bot2) {
      try {
        if (Math.floor(Math.random() * 2) + 1 == 1) {
          newGameData.player1 = {
            id: bot1._id.toString(),
            data: fs.readFileSync(p1),
          };
          newGameData.player2 = {
            id: bot2._id.toString(),
            data: fs.readFileSync(p2),
          };
        ...
        console.log(`newGame, gameId: ${newGameData.gameId}`);
  ...
  if (req.isAuthenticated() && req.user.role == "admin") {
    ...
    if (bot1 && bot2) {
      try {
        ...
        console.log(`newGame, gameId: ${newGameData.gameId}`);
        gameServerBridge.newGame(newGameData);
        gameStartedNotification.setResponseInstance(
          newMatch._id.toString(),
          res
        );
      } catch (err) {
        console.log(err);
    ...
    res.json({
      status: "error",
      message: "Lỗi quyền truy cập hoặc bạn chưa đăng nhập!",
    });
  }
};
