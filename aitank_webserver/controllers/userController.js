...
exports.add_post = async (req, res) => {
  ...
  let user = new User({
    ...
    hash: await bcrypt.hash(
      body.userName + body.role + configuration.SECRET,
      10
    ),
  });
  // user.hash = await bcrypt.hash(user.userName + user.role + configuration.SECRET, 10);
  ...
  User.findOne({ email: body.email })
    ...
    .then(async (user) => {
      ...
      user.hash = await bcrypt.hash(
        user.userName + body.role + configuration.SECRET,
        10
      );
      if (body.password != "") {
