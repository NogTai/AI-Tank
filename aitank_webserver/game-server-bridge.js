...
io.on("connection", async function (socket) {
  ...
  socket.on("gameOver", async function (data) {
    ...
    if ([1, 2, 3, 4].indexOf(parseInt(data.code)) != -1) {
      var gameReplay = fs.createWriteStream(
        __dirname + "/public/Replays/" + data.gameId + ".grl"
      );
  ...
  socket.on("gameStarted", async function (data) {
    ...
    if (sk) sk.json({ status: "ok", data: data.gameId });
    let match = await Match.findById(data.gameId);
    match.status = "started";
    await match.save();
    console.log(
      "a game started with id: " + data.gameId + " port " + data.gamePort
    );
