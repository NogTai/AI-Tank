...
var socket = io.connect(
  "http://" + configuration.SOCKET_HOST + ":" + configuration.SOCKET_PORT + "/",
  {
    reconnection: true,
  }
);
...
socket.on("newGame", function (data) {
  ...
  if (runningPort == 60000) runningPort = 50000;
  ...
  exec(
    "node Server.js -gid " +
      ...
      data.gameId +
      ".grl",
    { cwd: __dirname + "/Pack/Server", shell: true, windowsHide: false },
    function (err, stdout, stderr) {
      if (err) {
        ...
        socket.emit("gameOver", {
          gameId: data.gameId,
          code: 9,
          message: "error when execute server.js",
        });
      } else {
        if (fs.existsSync("./Pack/Server/Result/" + data.gameId)) {
          fs.readFile(
            "./Pack/Server/Result/" + data.gameId,
            "utf8",
            function (err, content) {
              if (err) {
                socket.emit("gameOver", {
                  gameId: data.gameId,
                  code: 9,
                  message: "error when read result file",
                });
              } else {
                let replay =
                  [1, 2, 3, 4].indexOf(parseInt(content)) != -1
                    ? fs.readFileSync(
                        "./Pack/Server/Replay/" + data.gameId + ".grl"
                      )
                    : {};
