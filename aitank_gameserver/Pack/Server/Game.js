...
module.exports = function Game(key1, key2, replayFilename, gameId) {
  var instance = this;
  // Get the map data
  ...
  this.m_bases[Enum.TEAM_1].push(
    new Base(
      this,
      0,
      Setting.BASE_POSITION_1[0][0],
      Setting.BASE_POSITION_1[0][1],
      Enum.TEAM_1,
      Enum.BASE_MAIN
    )
  );
  this.m_bases[Enum.TEAM_1].push(
    new Base(
      this,
      1,
      Setting.BASE_POSITION_1[1][0],
      Setting.BASE_POSITION_1[1][1],
      Enum.TEAM_1,
      Enum.BASE_SIDE
    )
  );
  this.m_bases[Enum.TEAM_1].push(
    new Base(
      this,
      2,
      Setting.BASE_POSITION_1[2][0],
      Setting.BASE_POSITION_1[2][1],
      Enum.TEAM_1,
      Enum.BASE_SIDE
    )
  );
  this.m_bases[Enum.TEAM_2].push(
    new Base(
      this,
      0,
      Setting.BASE_POSITION_2[0][0],
      Setting.BASE_POSITION_2[0][1],
      Enum.TEAM_2,
      Enum.BASE_MAIN
    )
  );
  this.m_bases[Enum.TEAM_2].push(
    new Base(
      this,
      1,
      Setting.BASE_POSITION_2[1][0],
      Setting.BASE_POSITION_2[1][1],
      Enum.TEAM_2,
      Enum.BASE_SIDE
    )
  );
  this.m_bases[Enum.TEAM_2].push(
    new Base(
      this,
      2,
      Setting.BASE_POSITION_2[2][0],
      Setting.BASE_POSITION_2[2][1],
      Enum.TEAM_2,
      Enum.BASE_SIDE
    )
  );
  ...
  function CheckConnection() {
    if (instance.m_player1Index != -1 && instance.m_player2Index != -1) {
      ...
      if (instance.m_player1Index != -1 && instance.m_player2Index == -1) {
        ...
        instance.m_player1Index == -1 &&
        instance.m_player2Index != -1
      ) {
  ...
  function CheckPickTank() {
    if (
      instance.m_tanks[Enum.TEAM_1].length == Setting.NUMBER_OF_TANK &&
      instance.m_tanks[Enum.TEAM_2].length == Setting.NUMBER_OF_TANK
    ) {
    } else {
      if (
        instance.m_tanks[Enum.TEAM_1].length < Setting.NUMBER_OF_TANK &&
    ...
    while (true) {
      ...
      if (command == Enum.COMMAND_PING) {
        ...
        if (this.m_state == Enum.STATE_TANK_PLACEMENT) {
          ...
          if (
            this.m_tanks[Enum.TEAM_1].length == Setting.NUMBER_OF_TANK &&
            this.m_tanks[Enum.TEAM_2].length == Setting.NUMBER_OF_TANK
          ) {
  ...
  this.GetUpdatePacket = function (force) {
    ...
    packet += Network.EncodeInt16(
      Setting.LOOPS_MATCH_END - instance.m_loopNumber
    );
  ...
  this.PickTank = function (player, type, x, y) {
    ...
    if (this.m_tanks[player].length == Setting.NUMBER_OF_TANK) {
      console.log(
        "Player " + player + " tried to pick more tank than he is allowed."
      );
      if (player == 1) {
        fs.writeFile("Result/" + gameId, "5", function (err) {
          if (!err) process.exit(0);
        });
      } else if (player == 2) {
        fs.writeFile("Result/" + gameId, "6", function (err) {
          if (!err) process.exit(0);
        });
      }
      return false;
    ...
    if (x < 0 || x > 22 || y < 0 || y > 22) {
      console.log(
        "Player " + player + " tried to place a tank out side of the map."
      );
      if (player == 1) {
        fs.writeFile("Result/" + gameId, "7", function (err) {
          if (!err) process.exit(0);
        });
      } else if (player == 2) {
        fs.writeFile("Result/" + gameId, "8", function (err) {
          if (!err) process.exit(0);
        });
      }
      return false;
    ...
    if (player == Enum.TEAM_1 && x > 7) {
      console.log(
        "Player " + player + " tried to place a tank out side of his base."
      );
    ...
    if (this.m_map[y * Setting.MAP_W + x] != Enum.BLOCK_GROUND) {
      console.log(
        "Player " + player + " tried to place a tank on an obstacles."
      );
      if (player == 1) {
        fs.writeFile("Result/" + gameId, "7", function (err) {
          if (!err) process.exit(0);
    ...
    for (var i = 0; i < this.m_tanks[player].length; i++) {
      if (
        this.m_tanks[player][i].m_x == x &&
        this.m_tanks[player][i].m_y == y
      ) {
        console.log(
          "Player " + player + " tried to place a tank on another tank."
        );
        if (player == 1) {
          fs.writeFile("Result/" + gameId, "7", function (err) {
            if (!err) process.exit(0);
  ...
  this.Fire = function (tank) {
    ...
    for (var i = 0; i < this.m_bullets[team].length; i++) {
      if (
        this.m_bullets[team][i].m_live == false &&
        this.m_bullets[team][i].m_needToAnnounceHit == false
      ) {
  ...
  this.SaveReplay = function () {
    if (replayFilename != null) {
      var replayString = "";
      for (var i = 0; i < replayData.length; i++) replayString += replayData[i];
  ...
  this.Update = function () {
    ...
    if (instance.m_matchResult != Enum.MATCH_RESULT_NOT_FINISH) {
      ...
      if (loopBeforeEnding == 0) {
        fs.writeFileSync("Result/" + gameId, instance.m_matchResult);
        instance.SaveReplay();
        return;
    ...
    if (instance.m_matchResult == Enum.MATCH_RESULT_NOT_FINISH) {
      ...
      if (
        instance.m_matchResult == Enum.MATCH_RESULT_NOT_FINISH &&
        instance.CheckWinLost()
      ) {
        instance.m_state = Enum.STATE_FINISHED;
        matchResultPacket = instance.GetMatchResultPackage();
        stateUpdatePacket = instance.GetStateUpdatePacket();
      ...
      //Should turn on sudden death mode?
        ...
        instance.m_loopNumber >= Setting.LOOPS_SUDDEN_DEATH
      ) {
        if (!instance.CheckWinLost120s()) {
          instance.SetSuddenDeathMode();
          stateUpdatePacket = instance.GetStateUpdatePacket();
        } else {
          instance.m_matchResult =
            instance.m_loser[Enum.TEAM_1] == false
              ? Enum.MATCH_RESULT_TEAM_1_WIN
              : Enum.MATCH_RESULT_TEAM_2_WIN;
          instance.m_state = Enum.STATE_FINISHED;
          matchResultPacket = instance.GetMatchResultPackage();
          stateUpdatePacket = instance.GetStateUpdatePacket();
        }
      ...
      //or time's up?
        ...
        instance.m_loopNumber >= Setting.LOOPS_MATCH_END
      ) {
  ...
  this.CheckWinLost = function () {
    ...
    for (var i = 0; i < this.m_bases[Enum.TEAM_1].length; i++) {
      if (
        this.m_bases[Enum.TEAM_1][i].m_type == Enum.BASE_MAIN &&
        this.m_bases[Enum.TEAM_1][i].m_HP == 0
      ) {
    ...
    for (var i = 0; i < this.m_bases[Enum.TEAM_2].length; i++) {
      if (
        this.m_bases[Enum.TEAM_2][i].m_type == Enum.BASE_MAIN &&
        this.m_bases[Enum.TEAM_2][i].m_HP == 0
      ) {
    ...
    var dieCount = 0;
    for (var i = 0; i < this.m_tanks[Enum.TEAM_1].length; i++) {
      if (this.m_tanks[Enum.TEAM_1][i].m_HP == 0) dieCount++;
    }
    ...
    dieCount = 0;
    for (var i = 0; i < this.m_tanks[Enum.TEAM_2].length; i++) {
      if (this.m_tanks[Enum.TEAM_2][i].m_HP == 0) dieCount++;
    }
        ...
        if (this.CheckWinLostTimeUp())
          this.m_matchResult =
            this.m_loser[Enum.TEAM_1] == false
              ? Enum.MATCH_RESULT_TEAM_1_WIN
              : Enum.MATCH_RESULT_TEAM_2_WIN;
        else this.m_matchResult = Enum.MATCH_RESULT_DRAW;
      } else if (this.m_loser[Enum.TEAM_1] == false) {
        this.m_matchResult = Enum.MATCH_RESULT_TEAM_1_WIN;
      } else {
        this.m_matchResult = Enum.MATCH_RESULT_TEAM_2_WIN;
    ...
    if (
      ...
      this.m_teamLostSuddenDeath != -1
    ) {
      if (this.m_loser[Enum.TEAM_1] && this.m_loser[Enum.TEAM_2]) {
        //both teams have base destroyed
        this.m_matchResult = Enum.MATCH_RESULT_DRAW;
  ...
  this.CheckWinLostTimeUp = function () {
    ...
    count2 = 0;
    for (var i = 0; i < this.m_tanks[Enum.TEAM_1].length; i++) {
      if (this.m_tanks[Enum.TEAM_1][i].m_HP > 0) count1++;
    }
    for (var i = 0; i < this.m_tanks[Enum.TEAM_2].length; i++) {
      if (this.m_tanks[Enum.TEAM_2][i].m_HP > 0) count2++;
    }
    if (count1 < count2) {
      this.m_loser[Enum.TEAM_1] = true;
      this.m_loser[Enum.TEAM_2] = false;
  ...
  this.ProcessTimeUp = function () {
    if (this.CheckWinLostTimeUp()) {
      ...
      //need more checking here, if no base or tank is destroyed -> DRAW_NEGATIVE
      var count = 0;
      for (var i = 0; i < this.m_bases[Enum.TEAM_1].length; i++) {
        if (this.m_bases[Enum.TEAM_1][i].m_HP == 0) {
          count++;
  ...
  this.SetSuddenDeathMode = function () {
    ...
    for (var i = 0; i < this.m_obstacles.length; i++) {
      if (this.m_obstacles[i].m_HP > 0) {
        ...
        this.m_map[
          this.m_obstacles[i].m_y * Setting.MAP_W + this.m_obstacles[i].m_x
        ] = Enum.BLOCK_GROUND;
      }
    }
    //change state
  ...
  this.GetBase = function (x, y) {
    for (var i = 0; i < this.m_bases[Enum.TEAM_1].length; i++) {
      if (
        this.m_bases[Enum.TEAM_1][i].m_x - 1 < x &&
        this.m_bases[Enum.TEAM_1][i].m_x + 1 > x &&
        this.m_bases[Enum.TEAM_1][i].m_y - 1 < y &&
        this.m_bases[Enum.TEAM_1][i].m_y + 1 > y
      )
    ...
    for (var i = 0; i < this.m_bases[Enum.TEAM_2].length; i++) {
      if (
        this.m_bases[Enum.TEAM_2][i].m_x - 1 < x &&
        this.m_bases[Enum.TEAM_2][i].m_x + 1 > x &&
        this.m_bases[Enum.TEAM_2][i].m_y - 1 < y &&
        this.m_bases[Enum.TEAM_2][i].m_y + 1 > y
      )
