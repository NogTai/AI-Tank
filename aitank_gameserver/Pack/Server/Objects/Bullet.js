...
module.exports = function Bullet(game, team, id) {
  ...
  this.CheckForCollision = function () {
    ...
    for (var i = 0; i < game.m_tanks[oppTeam].length; i++) {
      ...
      if (tempTank.m_HP > 0) {
        if (
          Math.abs(this.m_x - tempTank.m_x) <= 0.5 &&
          Math.abs(this.m_y - tempTank.m_y) <= 0.5
        ) {
          this.m_live = false;
    ...
    for (var i = 0; i < checkX; i++) {
      for (var j = 0; j < checkY; j++) {
        ...
        if (
          game.m_map[tempY * Setting.MAP_W + tempX] ==
            Enum.BLOCK_HARD_OBSTACLE ||
