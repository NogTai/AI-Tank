...
module.exports = function Tank(game, id, x, y, team, type) {
  ...
  this.Update = function () {
    if (this.m_HP > 0) {
      if (this.m_disabled <= 0) {
        if (
          commandTurn == Enum.DIRECTION_DOWN ||
          commandTurn == Enum.DIRECTION_RIGHT ||
          commandTurn == Enum.DIRECTION_LEFT ||
          commandTurn == Enum.DIRECTION_UP
        ) {
  ...
  this.CheckForCollision = function (newX, newY) {
    ...
    if (newX == roundedX && newY == roundedY) {
      squareNeedToCheckX.push(roundedX);
      squareNeedToCheckY.push(roundedY);
    } else if (newX != roundedX && newY == roundedY) {
      squareNeedToCheckX.push(roundedX);
      squareNeedToCheckY.push(roundedY);
      squareNeedToCheckX.push(roundedX + 1);
      squareNeedToCheckY.push(roundedY);
    } else if (newX == roundedX && newY != roundedY) {
      squareNeedToCheckX.push(roundedX);
      squareNeedToCheckY.push(roundedY);
      squareNeedToCheckX.push(roundedX);
      squareNeedToCheckY.push(roundedY + 1);
    } else if (newX != roundedX && newY != roundedY) {
      squareNeedToCheckX.push(roundedX);
      squareNeedToCheckY.push(roundedY);
      squareNeedToCheckX.push(roundedX + 1);
    ...
    for (var i = 0; i < squareNeedToCheckX.length; i++) {
      ...
      if (
        game.m_map[y * Setting.MAP_W + x] == Enum.BLOCK_WATER ||
    ...
    for (var i = 0; i < game.m_tanks[Enum.TEAM_1].length; i++) {
      ...
      if (
        Math.abs(newX - tempTank.m_x) < 1 &&
        Math.abs(newY - tempTank.m_y) < 1
      ) {
        return false;
      }
    }
