...
function GSActionPhase() {
  ...
  imgPowerUp[POWERUP_AIRSTRIKE] = g_graphicEngine.LoadImage(
    "Image/PowerUp/Airstrike.png"
  );
  ...
  this.Init = function () {
    if (init == false) {
      ...
      imgResultBoard[MATCH_RESULT_TEAM_1_WIN] = g_graphicEngine.LoadImage(
        "Image/Strings/Team1Win.png"
      );
      imgResultBoard[MATCH_RESULT_TEAM_2_WIN] = g_graphicEngine.LoadImage(
        "Image/Strings/Team2Win.png"
      );
      imgResultBoard[MATCH_RESULT_DRAW] = g_graphicEngine.LoadImage(
        "Image/Strings/Draw.png"
      );
      imgResultBoard[MATCH_RESULT_BAD_DRAW] = g_graphicEngine.LoadImage(
        "Image/Strings/BadDraw.png"
      );
  ...
  this.OnUpdatePacket = function (data) {
    ...
    while (true) {
      ...
      if (command == COMMAND_UPDATE_STATE) {
        ...
        if (
          this.m_state == STATE_SUDDEN_DEATH &&
          this.m_suddenDeathTime == -1
        ) {
        ...
        readOffset += this.ProcessUpdateObstacleCommand(
          packetCount,
          data,
          readOffset
        );
      } else if (command == COMMAND_UPDATE_TANK) {
        readOffset += this.ProcessUpdateTankCommand(
          packetCount,
          data,
          readOffset
        );
      } else if (command == COMMAND_UPDATE_BULLET) {
        readOffset += this.ProcessUpdateBulletCommand(
          packetCount,
          data,
          readOffset
        );
      } else if (command == COMMAND_UPDATE_BASE) {
        readOffset += this.ProcessUpdateBaseCommand(
          packetCount,
          data,
          readOffset
        );
      } else if (command == COMMAND_UPDATE_POWERUP) {
        readOffset += this.ProcessUpdatePowerUpCommand(
          packetCount,
          data,
          readOffset
        );
      } else if (command == COMMAND_MATCH_RESULT) {
        readOffset += this.ProcessMatchResultCommand(
          packetCount,
          data,
          readOffset
        );
        ...
        readOffset += this.ProcessUpdateInventoryCommand(
          packetCount,
          data,
          readOffset
        );
      } else if (command == COMMAND_UPDATE_STRIKE) {
        readOffset += this.ProcessUpdateStrikeCommand(
          packetCount,
          data,
          readOffset
        );
  ...
  this.ProcessUpdateTankCommand = function (time, data, originalOffset) {
    ...
    this.m_tanks[team][id].AddDataAnchor(
      time,
      x,
      y,
      dir,
  ...
  this.ProcessUpdateBulletCommand = function (time, data, originalOffset) {
    ...
    this.m_bullets[team][id].AddDataAnchor(
      time,
      type,
      x,
      y,
  ...
  this.ProcessUpdateInventoryCommand = function (time, data, originalOffset) {
    ...
    for (var i = 0; i < number1; i++) {
      this.m_inventory[index][TEAM_1][i] = DecodeUInt8(data, offset);
      offset++;
    }
    var number2 = DecodeUInt8(data, offset);
    offset++;
    for (var i = 0; i < number2; i++) {
      this.m_inventory[index][TEAM_2][i] = DecodeUInt8(data, offset);
      offset++;
    }
  ...
  this.Update = function (deltaTime) {
    ...
    if (g_inputEngine.m_mousePress == 1) {
      if (
        g_inputEngine.m_mouseX >= RW_BUTTON_X &&
        g_inputEngine.m_mouseX <= RW_BUTTON_X + RW_BUTTON_W &&
        g_inputEngine.m_mouseY >= RW_BUTTON_Y &&
        g_inputEngine.m_mouseY <= RW_BUTTON_Y + RW_BUTTON_H
      ) {
        playControlling = -1;
      } else if (
        g_inputEngine.m_mouseX >= FF_BUTTON_X &&
        g_inputEngine.m_mouseX <= FF_BUTTON_X + FF_BUTTON_W &&
        g_inputEngine.m_mouseY >= FF_BUTTON_Y &&
        g_inputEngine.m_mouseY <= FF_BUTTON_Y + FF_BUTTON_H
      ) {
        playControlling = 1;
      } else {
        playControlling = 0;
  ...
  this.Draw = function () {
      ...
      for (var j = 0; j < MAP_H; j++) {
        if (this.m_map[j * MAP_W + i] == BLOCK_WATER) {
          ...
          g_graphicEngine.Draw(
            ...
            BLOCK_SIZE,
            BLOCK_SIZE,
            i * BLOCK_SIZE + this.m_screenShakeX,
            j * BLOCK_SIZE + this.m_screenShakeY,
            BLOCK_SIZE,
            BLOCK_SIZE,
            MAP_WATER_ALPHA
          );
    ...
    g_graphicEngine.DrawFast(
      g_context,
      imgBackground,
      MAP_OFFSET_X + this.m_screenShakeX,
      MAP_OFFSET_Y + this.m_screenShakeY
    );
    ...
    if (
      this.m_matchResult != MATCH_RESULT_NOT_FINISH &&
      this.m_time >= this.m_matchResultTime
    ) {
      var boardY =
        -RESULT_BOARD_H + (this.m_time - this.m_matchResultTime) * 80;
      if (boardY > (MAP_H * BLOCK_SIZE) / 2 - RESULT_BOARD_H / 2) {
        boardY = (MAP_H * BLOCK_SIZE) / 2 - RESULT_BOARD_H / 2;
      }
      g_graphicEngine.DrawFast(
        g_context,
        imgResultBoard[this.m_matchResult],
        (MAP_W * BLOCK_SIZE) / 2 + this.m_screenShakeX - RESULT_BOARD_W / 2,
        boardY + this.m_screenShakeY
      );
    } else if (
      this.m_matchResult != MATCH_RESULT_NOT_FINISH &&
      this.m_matchResultTime <= 10
    ) {
      g_graphicEngine.DrawFast(
        g_context,
        imgResultBoard[this.m_matchResult],
        (MAP_W * BLOCK_SIZE) / 2 + this.m_screenShakeX - RESULT_BOARD_W / 2,
        (MAP_H * BLOCK_SIZE) / 2 + this.m_screenShakeY - RESULT_BOARD_H / 2
      );
    }
    ...
    for (var i = this.m_inventory.length - 1; i >= 0; i--) {
      if (this.m_time > this.m_inventory[i]["time"]) {
        for (var j = 0; j < this.m_inventory[i][TEAM_1].length; j++) {
          g_graphicEngine.DrawFast(
            g_context,
            imgPowerUp[this.m_inventory[i][TEAM_1][j]],
            TEAM_1_INVENTORY_X + j * 45,
            TEAM_1_INVENTORY_Y
          );
        }
        for (var j = 0; j < this.m_inventory[i][TEAM_2].length; j++) {
          g_graphicEngine.DrawFast(
            g_context,
            imgPowerUp[this.m_inventory[i][TEAM_2][j]],
            TEAM_2_INVENTORY_X + j * 45,
            TEAM_2_INVENTORY_Y
          );
        }
    ...
    if (this.m_time > this.m_suddenDeathTime && this.m_suddenDeathTime != -1) {
      ...
      var time =
        (((timeCorrectionFactor - this.m_time) * PACKET_PROCESS_INTERVAL) /
          1000) >>
        0;
      g_graphicEngine.DrawTextRGB(
        g_context,
        time,
        TIME_BOARD_X,
        TIME_BOARD_Y,
