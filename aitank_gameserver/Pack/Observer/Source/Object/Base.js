function Base(game, id, team, type) {
  ...
  imgBase[TEAM_1][BASE_MAIN] = g_graphicEngine.LoadImage(
    "Image/Base/Team 1/MainBase.png"
  );
  imgBase[TEAM_1][BASE_SIDE] = g_graphicEngine.LoadImage(
    "Image/Base/Team 1/SideBase.png"
  );
  imgBase[TEAM_2][BASE_MAIN] = g_graphicEngine.LoadImage(
    "Image/Base/Team 2/MainBase.png"
  );
  imgBase[TEAM_2][BASE_SIDE] = g_graphicEngine.LoadImage(
    "Image/Base/Team 2/SideBase.png"
  );

  var imgBaseD = new Array();
  imgBaseD[TEAM_1] = new Array();
  imgBaseD[TEAM_2] = new Array();
  imgBaseD[TEAM_1][BASE_MAIN] = g_graphicEngine.LoadImage(
    "Image/Base/Team 1/MainBaseD.png"
  );
  imgBaseD[TEAM_1][BASE_SIDE] = g_graphicEngine.LoadImage(
    "Image/Base/Team 1/SideBaseD.png"
  );
  imgBaseD[TEAM_2][BASE_MAIN] = g_graphicEngine.LoadImage(
    "Image/Base/Team 2/MainBaseD.png"
  );
  imgBaseD[TEAM_2][BASE_SIDE] = g_graphicEngine.LoadImage(
    "Image/Base/Team 2/SideBaseD.png"
  );
      ...
      if (previousAnchor.m_HP > 0 && tempAnchor.m_HP == 0) {
        ...
        for (var i = 1; i < 10; i++) {
          game.SpawnExplosion(
            time + i * 2,
            EXPLOSION_TANK,
            tempAnchor.m_x + 0.5 + Math.random() * 2 - 1,
            tempAnchor.m_y + 0.5 + Math.random() * 2 - 1
          );
        }
      }
    }
    ...
    if (shouldDraw) {
      if (this.m_HP > 0) {
        g_graphicEngine.DrawFast(
          g_context,
          imgBase[this.m_team][this.m_type],
          this.m_x * BLOCK_SIZE -
            BLOCK_SIZE / 2 +
        ...
        g_graphicEngine.FillCanvas(
          ...
          this.m_y * BLOCK_SIZE -
            ...
            g_gsActionPhase.m_screenShakeY +
            HP_BAR_OFFSET,
          BLOCK_SIZE * 2 * (this.m_HP / BASE_MAX_HP[this.m_type]),
          4
        );
      } else {
        g_graphicEngine.DrawFast(
          g_context,
          imgBaseD[this.m_team][this.m_type],
          this.m_x * BLOCK_SIZE -
            BLOCK_SIZE / 2 +
            g_gsActionPhase.m_screenShakeX,
          this.m_y * BLOCK_SIZE -
