function Strike(game, id, team) {
  ...
  imgAirPlane[TEAM_1] = g_graphicEngine.LoadImage(
    "Image/PowerUp/Airplane 1.png"
  );
  imgAirPlane[TEAM_2] = g_graphicEngine.LoadImage(
    "Image/PowerUp/Airplane 2.png"
  );
    ...
    if (tempAnchor.m_type == POWERUP_AIRSTRIKE && countDown == 0) {
      ...
      for (var i = 1; i < 5; i++) {
        game.SpawnExplosion(
          time + i,
          EXPLOSION_TANK,
          tempAnchor.m_x + Math.random() * 3 - 1.5,
          tempAnchor.m_y + Math.random() * 3 - 1.5
        );
      }
  ...
  this.Update = function (time) {
    ...
    if (prevAnchor) {
      ...
      this.m_countDown =
        prevAnchor.m_countDown +
        (nextAnchor.m_countDown - prevAnchor.m_countDown) * interpolateFactor;
    ...
    if (this.m_live) {
      if (this.m_countDown > 0 && this.m_type == POWERUP_AIRSTRIKE) {
        g_graphicEngine.DrawFast(
          g_context,
          imgAirPlane[this.m_team],
          this.m_x * BLOCK_SIZE +
            AIRPLANE_OFFSET +
            g_gsActionPhase.m_screenShakeX,
          -AIRPLANE_SPRITE_SIZE +
            (this.m_countDown * (CANVAS_H + AIRPLANE_SPRITE_SIZE)) /
              AIRSTRIKE_COUNTDOWN +
      ...
      whiteSmoke[0].Resume();
      whiteSmoke[0].m_x =
        this.m_x * BLOCK_SIZE + AIRPLANE_OFFSET + AIRPLANE_SMOKE_OFFSET_X_1;
      whiteSmoke[0].m_y =
        -AIRPLANE_SPRITE_SIZE +
        (this.m_countDown * (CANVAS_H + AIRPLANE_SPRITE_SIZE)) /
          AIRSTRIKE_COUNTDOWN +
        AIRPLANE_SMOKE_OFFSET_Y_1;

      whiteSmoke[1].Resume();
      whiteSmoke[1].m_x =
        this.m_x * BLOCK_SIZE + AIRPLANE_OFFSET + AIRPLANE_SMOKE_OFFSET_X_2;
      whiteSmoke[1].m_y =
        -AIRPLANE_SPRITE_SIZE +
        (this.m_countDown * (CANVAS_H + AIRPLANE_SPRITE_SIZE)) /
          AIRSTRIKE_COUNTDOWN +
        AIRPLANE_SMOKE_OFFSET_Y_2;
    } else {
      whiteSmoke[0].Pause();
      whiteSmoke[1].Pause();
    }
