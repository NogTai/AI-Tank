function Explosion(game, id) {
  ...
  imgExplosion[EXPLOSION_TANK] = g_graphicEngine.LoadImage(
    "Image/Explosion/1.png"
  );
  imgExplosion[EXPLOSION_CANNON] = g_graphicEngine.LoadImage(
    "Image/Explosion/2.png"
  );
  imgExplosion[EXPLOSION_OBSTACLE] = g_graphicEngine.LoadImage(
    "Image/Explosion/3.png"
  );
  imgExplosion[EXPLOSION_EMP] = g_graphicEngine.LoadImage(
    "Image/Explosion/4.png"
  );
  imgExplosion[EXPLOSION_BULLET] = g_graphicEngine.LoadImage(
    "Image/Explosion/5.png"
  );

  imgExplosion[EXPLOSION_CANNON_MUZZLE] = g_graphicEngine.LoadImage(
    "Image/Explosion/7.png"
  );
  imgExplosion[EXPLOSION_GUN_MUZZLE] = g_graphicEngine.LoadImage(
    "Image/Explosion/8.png"
  );
  ...
  this.Update = function (time) {
    ...
    if (anchor) {
      ...
      this.m_frame =
        ((time - anchor.m_time) * EXPLOSION_SPEED[this.m_type]) >> 0;
      if (this.m_frame > EXPLOSION_FRAME[this.m_type]) {
        this.m_live = false;
      } else {
        if (this.m_live == false) {
          fragEmitter.m_x =
            (this.m_x + 0.5) * BLOCK_SIZE + g_gsActionPhase.m_screenShakeX;
          fragEmitter.m_y =
            (this.m_y + 0.5) * BLOCK_SIZE + g_gsActionPhase.m_screenShakeY;
          ...
          if (
            this.m_type == EXPLOSION_TANK ||
            this.m_type == EXPLOSION_CANNON ||
            this.m_type == EXPLOSION_OBSTACLE
          ) {
  ...
  this.Draw = function () {
    if (this.m_live) {
      ...
      var sourceX =
        (this.m_frame % EXPLOSION_FRAME_W[this.m_type]) * spriteSize;
      var sourceY =
        ((this.m_frame / EXPLOSION_FRAME_W[this.m_type]) >> 0) * spriteSize;
      var spriteOffset = (BLOCK_SIZE - spriteSize * scale) * 0.5;

      g_graphicEngine.Draw(
        g_context,
        imgExplosion[this.m_type],
        sourceX,
        sourceY,
        spriteSize,
        spriteSize,
        this.m_x * BLOCK_SIZE + spriteOffset + g_gsActionPhase.m_screenShakeX,
        this.m_y * BLOCK_SIZE + spriteOffset + g_gsActionPhase.m_screenShakeY,
        spriteSize * scale,
        spriteSize * scale,
        1,
        this.m_flipX,
  ...
  this.IsFreeAt = function (time) {
    ...
    if (anchor) {
      if (time > anchor.m_time) {
        var frame =
          ((time - anchor.m_time) * EXPLOSION_SPEED[anchor.m_type]) >> 0;
