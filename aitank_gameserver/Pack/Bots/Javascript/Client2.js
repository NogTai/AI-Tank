...
function OnMessage(data) {
  ...
  while (true) {
    ...
    if (command == COMMAND_SEND_TEAM) {
      ...
      if (
        g_state == STATE_WAITING_FOR_PLAYERS &&
        state == STATE_TANK_PLACEMENT
      ) {
        g_state = state;
...
function GetOpponentTeam() {
  if (g_team == TEAM_1) return TEAM_2;
  else return TEAM_1;
}
...
function Update() {
  ...
  if (g_state == STATE_FINISHED) {
    if (
      (g_matchResult == MATCH_RESULT_TEAM_1_WIN && GetMyTeam() == TEAM_1) ||
      (g_matchResult == MATCH_RESULT_TEAM_2_WIN && GetMyTeam() == TEAM_2)
    ) {
      console.log("I WON. I WON. I'M THE BEST!!!");
    } else if (
      (g_matchResult == MATCH_RESULT_TEAM_2_WIN && GetMyTeam() == TEAM_1) ||
