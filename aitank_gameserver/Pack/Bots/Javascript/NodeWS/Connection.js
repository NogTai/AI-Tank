...
function Connection(socket, parentOrUrl, callback) {
  ...
  if (!this.server) {
    connectEvent =
      socket.constructor.name === "CleartextStream"
        ? "secureConnect"
        : "connect";
...
Connection.prototype.sendText = function (str, callback) {
  if (this.readyState === this.OPEN) {
    ...
    this.emit(
      "error",
      new Error(
        "You can't send a text frame until you finish sending binary frames"
      )
    );
  }
...
Connection.prototype.beginBinary = function () {
  if (this.readyState === this.OPEN) {
    ...
    this.emit(
      "error",
      new Error(
        "You can't send more binary frames until you finish sending the previous binary frames"
      )
    );
  }
  this.emit("error", new Error("You can't write to a non-open connection"));
...
Connection.prototype.readHandshake = function () {
  ...
  if (this.buffer.length > Connection.maxBufferLength) {
    ...
    this.socket.end(
      this.server ? "HTTP/1.1 400 Bad Request\r\n\r\n" : undefined
    );
    return false;
  }
